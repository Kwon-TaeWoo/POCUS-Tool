...
'''
class VisionTransformer_classifier(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        super(VisionTransformer_classifier, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = config.classifier
        self.transformer = Transformer(config, img_size, vis)
    ...
    def forward(self, x):
        ...
        #classifier logits
        cls_logits = self.classifier_head(x.mean(dim=2).mean(dim=2))
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            # Initialize classifier head with random weights
...
class FCDenseNet(nn.Module):
    ...
    def __init__(self, input_channel, num_class, ratio_dropout = 0.2):
        super().__init__()
...
class FCN8(nn.Module):
    def __init__(self, input_channel, n_class, t=2):
        img_ch = input_channel
        numclass = n_class
        self.__cepoch = 0
        super(FCN8, self).__init__()
        self.conv11 = nn.Conv2d(in_channels=img_ch, out_channels=64, kernel_size=3, padding=1)
        ...
        self.tranconv2 = nn.ConvTranspose2d(in_channels=512, out_channels=256, kernel_size=4, stride=2,  padding=1)

        self.tranconv3 = nn.ConvTranspose2d(in_channels=256, out_channels=numclass, kernel_size=16, stride=8, padding=4)
        self.finalConv = nn.Conv2d(in_channels=numclass,
                                   out_channels=numclass, kernel_size=1, stride=1,
                                   padding=0, bias=True)
        self.softmax = nn.LogSoftmax(dim=1)
